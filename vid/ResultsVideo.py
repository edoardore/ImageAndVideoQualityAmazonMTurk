...
hit_id_list = pickle.load(open('videoshitid.p', 'rb'))
for tuple in hit_id_list:
    hit_id = tuple[0]
    video = tuple[1][53:len(tuple[1])]
    ...
    if numAssignmentSubmitted >= MinAssignments:
        for assignment in assignments:
            ...
            if (freetext[4] == 'true'):
                ...
                sex = 'F'
            sqlSelect = "SELECT * FROM vidquality WHERE WORKER_ID='%s' AND VIDEO_FILE='%s' AND QUALITY='%s' AND AGE='%s' AND SEX='%s' AND RESOLUTION='%s'" % (
                WorkerId, video, quality, age, sex, resolution)
            sqlInsert = "INSERT INTO vidquality(WORKER_ID, VIDEO_FILE, QUALITY, AGE, SEX, RESOLUTION)VALUES" \
                        "('%s', '%s', '%s', '%s', '%s', '%s')" \
                        % (WorkerId, video, quality, age, sex, resolution)
            try:
